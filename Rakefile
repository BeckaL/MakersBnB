...
task :setup do
  ...
  ['makers_bnb', 'makers_bnb_test'].each do |database|
    ...
    connection.exec("CREATE TABLE bookings(booking_id SERIAL PRIMARY KEY, guest_id INTEGER REFERENCES users(user_id), host_id INTEGER REFERENCES users(user_id), listing_id INTEGER REFERENCES listings(listing_id), date DATE, approved VARCHAR);")
  end
end
