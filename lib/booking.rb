class Booking
  ...
  def self.create(guest_email:, listing_id:, date:)
    ...
    guest_id = DatabaseConnection.query("SELECT user_id FROM users
    WHERE email = '#{guest_email}'").first["user_id"].to_i
    host_id = DatabaseConnection.query("SELECT user_id FROM listings
    WHERE listing_id = '#{listing_id}'").first["user_id"].to_i
    DatabaseConnection.query("INSERT INTO bookings
    (guest_id, host_id, listing_id, date)
  ...
  def self.select_by_host(host_email)
    DatabaseConnection.setup
    host_id = DatabaseConnection.query("SELECT user_id FROM users
    WHERE email = '#{host_email}'").first["user_id"]
    booking_requests = all
  ...
  def self.convert_user_id(id)
    DatabaseConnection.setup
    DatabaseConnection.query("SELECT email FROM users
    WHERE user_id = '#{id}'").first["email"]
  end

  def self.convert_listing_id(list_id)
    DatabaseConnection.setup
    listing_name = DatabaseConnection.query("SELECT name FROM listings
    WHERE listing_id = '#{list_id}'").first["name"]
